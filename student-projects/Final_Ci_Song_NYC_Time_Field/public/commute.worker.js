// app/workers/commute.worker.ts
var stationCount = 0;
var cellCenters = [];
var stationCoordinates = [];
var access;
var adjacency = [];
var driveReverseAdjacency = [];
var matrices;
var graphs = {};
var graphAdjacency = {};
var referenceSpeeds;
var matrixUnit = 0.1;
var unreachable = 65535;
var entryOverhead = 2;
var exitOverhead = 1;
...
var MinHeap = class {
  ...
  pop() {
    ...
    while (true) {
      ...
      const right = left + 1;
      let smallest = index;
      if (left < this.values.length && this.values[left][0] < this.values[smallest][0]) {
        smallest = left;
      }
      if (right < this.values.length && this.values[right][0] < this.values[smallest][0]) {
        smallest = right;
      }
...
function walkingDistances(origin) {
...
function drivingDistances(destination) {
  const distance = new Array(cellCenters.length).fill(Number.POSITIVE_INFINITY);
  const heap = new MinHeap();
...
function walkingRoute(origin, destination) {
  ...
  heap.push([0, origin]);
  while (heap.size) {
    const current2 = heap.pop();
    if (!current2) break;
    const [minutes, node] = current2;
    if (minutes !== distance[node] || minutes > 90) continue;
...
function reconstructSubwayRoute(scenario, originStation, destinationStation) {
  ...
  const previousEdge = new Array(stateCount).fill(null);
  const heap = new MinHeap();
  for (const state2 of graph.station_states[String(originStation)] || []) {
    const key = `${graph.states[state2].route}:${graph.states[state2].direction}`;
    const wait = (graph.headways[key] || 10) / 2;
    distance[state2] = wait;
    heap.push([wait, state2]);
  }
  ...
  while (heap.size) {
    const current = heap.pop();
    if (!current) break;
    const [minutes, state2] = current;
    if (minutes !== distance[state2]) continue;
    if (destinations.has(state2)) {
      target = state2;
      break;
    }
    for (const edge of graphAdjacency[scenario][state2]) {
      const candidate = minutes + edge.minutes;
      if (candidate < distance[edge.to]) {
        distance[edge.to] = candidate;
        previousState[edge.to] = state2;
  ...
  const initialWait = (graph.headways[`${root.route}:${root.direction}`] || 10) / 2;
...
function route(payload) {
  ...
  const waitMinutes = subway.initialWait + subway.edges.reduce((sum, edge) => sum + edge.wait_minutes, 0);
  ...
  for (const edge of subway.edges) {
    ...
    if (edge.kind === "ride" && routeSequence[routeSequence.length - 1] !== edge.route) {
  ...
  const walkCellSequence = direct ? walkingRoute(payload.cell, payload.destinationCell) : [];
  const straightDistanceKm = haversineKm(
    cellCenters[payload.cell],
    cellCenters[payload.destinationCell]
  );
  const walkingMinutes = direct ? Math.max(
    0,
    walkingDistances(payload.destinationCell)[payload.cell]
  ) : Math.max(0, payload.originWalk - entryOverhead) + Math.max(0, payload.destinationWalk - exitOverhead);
  const networkDistanceKm = walkingMinutes * 80 / 1e3 + rideDistanceKm + transferMinutes * 80 / 1e3;
  return {
    type: "route",
    ...
    circuityRatio: Number(
      (networkDistanceKm / Math.max(straightDistanceKm, 1e-3)).toFixed(2)
    ),
    edges: subway.edges
  };
}
function compute(destinationCell, scenario) {
  ...
  for (let cell = 0; cell < cellCenters.length; cell += 1) {
    ...
    for (let entryIndex = 0; entryIndex < candidateCount; entryIndex += 1) {
      ...
      for (const destination of destinationCandidates) {
        const matrixValue = matrix[originStation * stationCount + destination.station];
